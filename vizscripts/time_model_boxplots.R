df = read.csv('~/Dropbox/python/clean/results/time_collated_results.tsv', sep = '\t')
...
p <- ggplot(data = df, aes(x = as.factor(corpus), y = accuracy)) + theme_bw() +
  ...
  scale_y_continuous('', labels = percent, breaks = c(.8, .825, .85, .875, .9, .925, .95)) +
  theme(text = element_text(size = 18, family = "Baskerville"), panel.border = element_blank()) +
  theme(axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.title.x = element_blank())

tiff("/Users/tunder/Dropbox/python/clean/boxplot3.tiff", height = 4, width = 6.4, units = 'in', res=400)
plot(p)
dev.off()
plot(p)